...
public int userID = 0;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		...
		View root = inflater.inflate(R.layout.activity_profile, null);
		
		setupActionBar();
		Spinner spinner = (Spinner) getActivity().findViewById(R.id.age_spinner);
		ArrayAdapter<CharSequence> adapter = ArrayAdapter.createFromResource(getActivity(), R.array.age_ranges, android.R.layout.simple_spinner_item);
	...
	private class JsonParser extends AsyncTask<Void,Void,JSONObject>{
		...
		protected void onPostExecute(JSONObject jsonObject){
			...
			try {
				...
				EditText editText = (EditText) getActivity().findViewById(R.id.edit_username);
				editText.setText(jsonObject.getString("Username"));
				editText = (EditText) getActivity().findViewById(R.id.edit_password);
				editText.setText(jsonObject.getString("Password"));
				editText = (EditText) getActivity().findViewById(R.id.edit_password_confirm);
				editText.setText(jsonObject.getString("Password"));
				editText = (EditText) getActivity().findViewById(R.id.edit_firstname);
				editText.setText(jsonObject.getString("FirstName"));
				editText = (EditText) getActivity().findViewById(R.id.edit_lastname);
				editText.setText(jsonObject.getString("LastName"));
				editText = (EditText) getActivity().findViewById(R.id.edit_home_address);
				editText.setText(new JSONObject(jsonObject.getString("HomeAddress")).getString("NumberAndStreet"));
				editText = (EditText) getActivity().findViewById(R.id.edit_home_zip);
				editText.setText(new JSONObject(jsonObject.getString("HomeAddress")).getString("ZipCode"));
				editText = (EditText) getActivity().findViewById(R.id.edit_work_address);
				editText.setText(new JSONObject(jsonObject.getString("WorkAddress")).getString("NumberAndStreet"));
				editText = (EditText) getActivity().findViewById(R.id.edit_work_zip);
				editText.setText(new JSONObject(jsonObject.getString("WorkAddress")).getString("ZipCode"));
				editText = (EditText) getActivity().findViewById(R.id.edit_gender);
				editText.setText(jsonObject.getString("Gender"));
				Spinner spinner = (Spinner) getActivity().findViewById(R.id.age_spinner);
				ArrayAdapter<CharSequence> adapter = (ArrayAdapter<CharSequence>) spinner.getAdapter();
				int pos = adapter.getPosition(jsonObject.getString("AgeRange"));
				spinner.setSelection(pos);
				CheckBox checkBox = (CheckBox) getActivity().findViewById(R.id.checkbox_swift911);
				if (jsonObject.getBoolean("SignupSwift") == true)
					checkBox.setChecked(true);
				else
					checkBox.setChecked(false);
				//System.out.println(profile.signup_swift);
				editText = (EditText) getActivity().findViewById(R.id.edit_phone);
				editText.setText(jsonObject.getString("PhoneNumber"));
				editText = (EditText) getActivity().findViewById(R.id.edit_email);
				editText.setText(jsonObject.getString("EmailAddress"));
				checkBox = (CheckBox) getActivity().findViewById(R.id.checkbox_submit_gps);
	...
	public void saveProfileData(View view) {
		EditText password1 = (EditText) getActivity().findViewById(R.id.edit_password);
		EditText password2 = (EditText) getActivity().findViewById(R.id.edit_password_confirm);
